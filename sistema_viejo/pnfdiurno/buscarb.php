          ...
          <?php
            ...
            if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                ...
                <td>'.$row["plan"].'</td> 
                <td>'.$row["actividad"].'</td> 
                <td><a href="nominasb.php?id='.$row['id'].'" data-toggle="tooltip" title="Mostrar datos" class="btn btn-sm btn-primary"> <span class="glyphicon glyphicon-eye-open"></span></span> </a></td><br>                          
              </tr>';
            }
          } 
          $conn->close();
