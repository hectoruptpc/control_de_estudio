...
class PDF extends FPDF
{
	...
	function Encabezado($carrera_a1)
	{
		$this->AddPage();
		date_default_timezone_set('America/Caracas');
        ...
        $this->Cell(190, 5, utf8_decode("CONSTANCIA DE INSCRIPCIÓN"), 0, 0, 'C', 0);
...
////////////////////////////////////////////////////////////////////////	   
		...
		if($fila['trayecto']==1 && $t==1){

			$pdf->SetFont('Arial','B',8);			
			$pdf->SetXY(15,35+$X1B);
			$pdf->Cell(180, 4, utf8_decode("Trayecto:  1"), 0, 0, 'L', 0);
			$t=2;
			$pdf->SetFont('Arial','',8);
		...
		if($fila['trayecto']==2 && $t==2){

			$pdf->SetFont('Arial','B',8);				
			$pdf->SetXY(15,35+$X1B);
			$pdf->Cell(180, 4, utf8_decode("Trayecto:  2"), 0, 0, 'L', 0);
			$t=3;
		...
		if($fila['trayecto']==3 && $t==3){
			$pdf->SetFont('Arial','B',8);				
			$pdf->SetXY(15,35+$X1B);
			$pdf->Cell(180, 4, utf8_decode("Trayecto:  3"), 0, 0, 'L', 0);
			$t=4;
		...
		if($fila['trayecto']==4 && $t==4){

			$pdf->SetFont('Arial','B',8);					
			$pdf->SetXY(15,35+$X1B);
			$pdf->Cell(180, 4, utf8_decode("Trayecto:  4"), 0, 0, 'L', 0);
			$t=5;
...
////////////////////////////////////////////////////////////////////////	   
           ...
           if($t==0){
				$pdf->SetFont('Arial','B',8);	
				$pdf->SetXY(15,35+$X1B);
				//$pdf->Cell(180, 4, "Trayecto:  0 -----RESULTADO:   trayecto: ".$trayecto[$i]." - materias_aprobadas: ".$materias_aprobadas." - cantidad_materias: ".$cantidad_materias, 0, 0, 'L', 0);
				$pdf->Cell(180, 4, utf8_decode("Trayecto:  0"), 0, 0, 'L', 0);			 	
				$t=3;
				$X1B=$X1B+4;
			}
