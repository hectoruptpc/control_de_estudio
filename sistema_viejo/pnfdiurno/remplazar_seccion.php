...
<!--       <th width="1%">cod_mat</th>
      <th width="50%">cod_doc</th>   
      <th width="1%">lapso</th>                                     
      <th width="10%">seccion</th>
      <th width="10%">seccion2</th> 
...
if($accion=="buscar") {                                                                                                                                     
  $sql = "SELECT notas.codigo,notas.cod_mat,notas.cod_doc,notas.lapso,notas.seccion,alumno.nombre FROM notas,alumno where notas.cod_mat='".$cod_mat."' and notas.seccion='".$seccion."' and notas.lapso='".$lapso."' and notas.cod_doc='".$cod_doc."' and notas.codigo=alumno.cedula";
...
$result = mysqli_query($conn, $sql);
 ...
 /////////****************////////////////         
  $sql = "UPDATE notas SET `seccion` = REPLACE(`seccion`, '".$seccion."', '".$seccion2."') WHERE `cod_mat`='".$cod_mat."' AND `cod_doc`='".$cod_doc."' AND `lapso`='".$lapso."'";

  $result = $conn->query($sql);

  $sql = "UPDATE agregarseccion SET `seccion` = REPLACE(`seccion`, '".$seccion."', '".$seccion2."') WHERE `cod_mat`='".$cod_mat."' AND `cod_doc`='".$cod_doc."' AND `lapso`='".$lapso."'";

  $result = $conn->query($sql);
 /////////****************//////////////// 


  $sql = "SELECT notas.codigo,notas.cod_mat,notas.cod_doc,notas.lapso,notas.seccion,alumno.nombre FROM notas,alumno where notas.cod_mat='".$cod_mat."' and notas.seccion='".$seccion2."' and notas.lapso='".$lapso."' and notas.cod_doc='".$cod_doc."' and notas.codigo=alumno.cedula";
