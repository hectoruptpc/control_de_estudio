...
if ($job == 'get_companies'){ 
    ...
    $query = "SELECT * FROM lismat ORDER BY pensum";
    ...
    if (!$query){
      ...
      while ($row = mysqli_fetch_array($query)){
        ...
        $functions .= '<li class="function_edit"><a href="Formulario_lismat_form_editar.php?action=editar&id='.$row['id'].'" data-id="'. $row['id'].'" data-name="' . $row['pensum'] . '"  style="background: #FF7D00"><span>Edit</span></a></li>';
    ...
    if ($id == ''){
      ...
      $query = "SELECT * FROM lismat WHERE id = '" . mysqli_real_escape_string($db_connection, $id) ."'";
      $query = mysqli_query($db_connection, $query);
      if (!$query){
        $result  = 'error';
        $message = 'query error';
