<?php
// Archivo: historial_desglozado_tsu.php
...
try {
    ...
    if ($estudiante_info) {
        ...
        if ($carrera) {
            ...
            // Filtrar solo trayectos 0-2 para TSU
            $historial_notas = array_filter($historial_completo, function($nota) {
                return $nota['trayecto'] >= 0 && $nota['trayecto'] <= 2;
            });
        }
    }
} catch (Exception $e) {
...
try {
    ...
    $pdf->Cell(0, 7, 'HISTORIAL DESGLOZADO DE NOTAS - TSU (TRAYECTOS 0-2)', 0, 1, 'C');
    $pdf->Ln(2);
    ...
    // Columnas para TSU (T0, T1, T2)
    $pdf->Cell(15, 5, 'TRAYECTO 0', 1, 0, 'C', true);
    $pdf->Cell(15, 5, 'TRAYECTO 1', 1, 0, 'C', true);
    $pdf->Cell(15, 5, 'TRAYECTO 2', 1, 0, 'C', true);
    ...
    if ($total_inscripciones > 0) {
        ...
        foreach ($materias_agrupadas as $codigo => $materia) {
            ...
            foreach ($materia['inscripciones'] as $index => $inscripcion) {
                ...
                // Mostrar notas de TSU (T0, T1, T2)
                $pdf->Cell(15, 5, $inscripcion['trayecto_0'] !== null ? $inscripcion['trayecto_0'] : '-', 1, 0, 'C');
                $pdf->Cell(15, 5, $inscripcion['trayecto_1'] !== null ? $inscripcion['trayecto_1'] : '-', 1, 0, 'C');
                $pdf->Cell(15, 5, $inscripcion['trayecto_2'] !== null ? $inscripcion['trayecto_2'] : '-', 1, 0, 'C');
        ...
        $pdf->Cell(190, 10, t('NO HAY HISTORIAL DE NOTAS TSU (TRAYECTOS 0-2)'), 1, 1, 'C');
    }

    // Estadísticas
    $pdf->Ln(5);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(100, 5, 'ESTADÍSTICAS TSU DESGLOZADAS:', 0, 0);
    $pdf->SetFont('Arial', '', 8);
    ...
    $pdf->Cell(60, 5, 'Total Materias TSU: ' . $total_materias, 0, 0);
    ...
    $aprobadas_t0 = 0; $aprobadas_t1 = 0; $aprobadas_t2 = 0;
    foreach ($historial_notas as $nota) {
        if ($nota['trayecto_0'] !== null && $nota['trayecto_0'] >= 12) $aprobadas_t0++;
        if ($nota['trayecto_1'] !== null && $nota['trayecto_1'] >= 12) $aprobadas_t1++;
        if ($nota['trayecto_2'] !== null && $nota['trayecto_2'] >= 12) $aprobadas_t2++;
    }
    
    $pdf->Cell(60, 5, 'Aprob. T0: ' . $aprobadas_t0 . ' / T1: ' . $aprobadas_t1 . ' / T2: ' . $aprobadas_t2, 0, 1);
    ...
    $nombre_archivo = 'Historial_Desglozado_TSU_' . preg_replace('/[^a-zA-Z0-9]/', '_', $estudiante_cedula) . '_' . date('Ymd_His') . '.pdf';
