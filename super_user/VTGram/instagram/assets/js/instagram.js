function Instagram(){
	...
	this.init= function(){
        ...
        $(".menu .mb-icon-menu").hover(function(){
            _that = $(this);
            _main = $(".menu-main");
        ...
        $('.dialog-upload').click(function() {
            ...
            var fm = $('<div/>').dialogelfinder({
                ...
                getFileCallback : function(files, fm) {
                    _that.parents(".form-group").find("input").val(files.url);
                },
                commandsOptions : {
                    getfile : {
                        oncomplete : 'close',
        ...
        $(document).on('click', '.btnUpdateGroups', function(){
            ...
            $.post(PATH+"Instagram_account/ajax_get_groups", _data, function(result){
                alert(result['txt']);
            },'json');
        });
        ...
        $(document).on("click", ".btnSearchScrap", function(){
            ...
            if(!_that.hasClass('disable')){
                _that.addClass('disable');
                $(".list-search-feed").html("");
                self.getLoading(".list-search-feed");
        ...
        $(document).on('click', '.btnSaveScheduleComment1', function(){
            self.schedule($(this), 'comment1');
        });

        $(document).on('click', '.btnSaveScheduleLike1', function(){
            self.schedule($(this), 'like1');
        });
        ...
        $(document).on('click', '.btnActionEditItem', function(){
            _that   = $(this);
            _action = _that.data("action");
            _id     = _that.parents(".item").data("id");
            _data   = _data + '&' + $.param({token:token, id: _id});
            if(!_that.hasClass('disable')){
                _that.addClass('disable');
                $('#modal-edit-page').remove(); 
        ...
        $(document).on('click', '.btnActionList', function(){
            ...
            _type   = _that.data("type");
            _data   = _that.closest("form").serialize();
            _data   = _data + '&' + $.param({token:token, action: _action, type: _type});
            if(!_that.hasClass('disable')){
                _that.addClass('disable');
            ...
            if(!_that.hasClass('disable')){
                ...
                $.post(PATH + "Instagram_follow/ajax_action_follow", _data, function(result){
                    if(result.st == "success"){
                        switch(_action){
                            case "follow":
                                _that.removeClass("btn-danger btn-success").addClass("btn-danger").data("action", "unfollow").text(unfollow);
                                break;
                            case "unfollow":
                                _that.removeClass("btn-danger btn-success").addClass("btn-success").data("action", "follow").text(follow);
                                break;  
                        }                   
                    }else{
                        alert(result.txt);
    ...
    this.AjaxLoadList = function(){
        ...
        if(_page != -1){
            self.delLoading();
            self.getLoading(".list_load");
            if(!_that.hasClass('disable')){
                _that.addClass('disable');
                $.post(_action, _data, function(result){
                    _html = jQuery.parseJSON(result['result']);
                    $(_html).insertBefore( ".list_load");
    ...
    this.getLoading = function(element){
        _html = '<div class="custom-loading"><img src="'+BASE+'assets/img/loader.gif"></div>';
