<div class="wrap">
	<div class="section-dashboard">
		<form class="formList">
	        <div class="tab-head">
				<span class="title"> <i class="fa fa-history" aria-hidden="true"></i> <?=l('history')?></span>
				<div class="right">
					<div class="btn-group">
				        <button type="button" class="btn btn-warning dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><?=l('action')?>
				            <span class="fa fa-caret-down"></span></button>
				        <ul class="dropdown-menu">
				            <li><a class="btnActionModule" data-action="delete"  data-confirm="<?=l('confirm-delete')?>" href="javascript:void(0);"><?=l('Delete All')?></a></li>
				        </ul>
				    </div>
				</div>
			</div>
			<div class="form-inline" style="display:inline-block;">
			    <div class="form-group">
			        <select class="form-control" name="type">
			        	...
			        	<option value="like" <?=(get('type') == "like")?"selected":""?> ><?=l('like')?></option>
			        	<option value="comment" <?=(get('type') == "comment")?"selected":""?> ><?=l('comment')?></option>
			        </select>
			    </div>
			...
			<div class="table-responsive mt15">
	            <table class="table table-bordered table table-striped">
	                ...
	                <tr class="item" data-id="<?=$row->id?>">
	                    ...
	                    <td><?=$row->type?></td>
	                    <td><a href="<?=(($row->type=="like" || $row->type=="comment")?"https://instagram.com/p/":"https://instagram.com/")?><?=$row->name?>" target="_blank"><?=$row->name?></a></td>
