...
class Schedule extends MX_Controller {
	...
	public function index(){
		...
		$data = array(
			'accounts' => $accounts,
			'countSchedule' => $this->model->countSchedules(),
			'savepost' => $savepost
		);
	...
	public function ajax_page(){
		$result = $this->model->getSchedules();
        ms(array(
			'st' 	 => 'success',
	...
	public function ajax_action_multiple(){
		switch (post("action")) {
			case 'delete':
				$this->db->delete(SCHEDULE_TB, "uid = '".session("uid")."' AND schedule_type = '".post('type')."'");
				break;
			
			case 'cancel':
				$this->db->update(SCHEDULE_TB, array("status" => 5), "uid = '".session("uid")."' AND schedule_type = '".post('type')."'");
				break;

			case 'repost':
				$this->db->update(SCHEDULE_TB, array("status" => 4), "uid = '".session("uid")."' AND schedule_type = '".post('type')."'");
				break;
		}
	...
	public function ajax_post_now(){
		...
		switch (post('type')) {
			case 'video':
				...
				$data = array(
					"type"        => "video",
					"description" => html_convert(post("description")),
					"url"         => post("media")
				);
				break;
	...
	public function ajax_schedule_comment1(){
		ini_set('max_execution_time', 3000);
		ini_set('max_input_vars', 10000);
	...
	public function ajax_schedule_comment(){
		...
		switch (post("type")) {
			case 2:
				if(post("location") == ""){
					$json[] = array(
						"type"   => "list_accounts",
						"text"   => l('Please enter location')
					);
				}
				$data_type = post("location");
				break;
			...
			case 3:
				if(post("usernames") == ""){
					$json[] = array(
						"type"   => "list_accounts",
						"text"   => l('Please enter usernames')
					);
				}
				$data_type = post("usernames");
				break;
		...
		if(!empty($json)){
			...
			foreach ($accounts as $key => $row) {
				...
				if(count($value) == 2){
					$data["social"]        = 'instagram';
					$data["schedule_type"] = 'auto_comment';
					$data["account"]       = $value[0];
					$data["name"]          = $value[1];
					$data["uid"]           = session("uid");
					...
					$data["image"]         = html_convert(post("description"));
					$data["repeat_post"]   = 1;
					...
					$check = $this->model->get("*", SCHEDULE_TB, "schedule_type = 'comment' AND uid = '".session("uid")."' AND account = '".$value[0]."'");
					if(empty($check)){
						$this->db->insert(SCHEDULE_TB, $data);
					}else{
	...
	public function ajax_schedule_like1(){
		ini_set('max_execution_time', 3000);
		ini_set('max_input_vars', 10000);
		...
		if(!empty($json)){
			...
			foreach ($accounts as $key => $row) {
				...
				if(count($value) == 2){
					...
					$check = $this->model->get("*", SCHEDULE_TB, "schedule_type = 'like' AND uid = '".session("uid")."' AND account = '".$value[0]."'");
					if(empty($check)){
						$this->db->insert(SCHEDULE_TB, $data);
					}else{
						$this->db->update(SCHEDULE_TB, $data, "id = '".$check->id."'");
		...
		if(!empty($json)){
			...
			foreach ($accounts as $key => $row) {
				...
				if(count($value) == 2){
					...
					$data["deplay"]        = (int)post("deplay");
					$data["maximum"]       = (int)post("maximum");
					$data["description"]   = html_convert(post("description"));
					$data["repeat_post"]   = 1;
					$data["repeat_time"]   = $deplay;
					$data["repeat_end"]    = "2030-09-09 00:00:00";
					$data["changed"]       = NOW;
	...
	public function cronjob3(){
	    ...
	    ->where('status != ', 3)
	    ->where('status != ', 5)
	    ->where('schedule_type', 'auto_comment')
	    ->where('time_post <= ', NOW)
	    ->get()->result();
