...
try {
    ...
    $pdf->Cell(0, 7, 'HISTORIAL DESGLOZADO DE NOTAS - TSU', 0, 1, 'C');
    $pdf->Ln(2);
    ...
    // Datos de la Tabla agrupados por Trayecto - SOLO TRAYECTOS 0-2
    if (!empty($materias_carrera_array)) {
        ...
        $pdf->Cell(190, 10, t('NO HAY MATERIAS TSU REGISTRADAS (TRAYECTOS 0-2)'), 1, 1, 'C');
        $total_creditos = 0;
        $creditos_aprobados = 0;
    }
    ...
    $pdf->Cell(100, 5, 'RESUMEN TSU:', 0, 0);
    $pdf->SetFont('Arial', '', 8);
    ...
    $pdf->Cell(60, 5, 'Total Creditos TSU: ' . $total_creditos, 0, 0);
    $pdf->Cell(60, 5, 'Creditos Aprobados TSU: ' . $creditos_aprobados, 0, 1);

    $pdf->Cell(60, 5, t('Índice de Rendimiento Académico TSU: N/D'), 0, 0);
    $pdf->SetX(110);
    ...
    $nombre_archivo = 'Historial_TSU_' . preg_replace('/[^a-zA-Z0-9]/', '_', $estudiante_cedula) . '_' . date('Ymd_His') . '.pdf';
