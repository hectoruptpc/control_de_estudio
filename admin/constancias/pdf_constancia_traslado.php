...
$pdf->SetFont('Arial','B',12); $pdf->Cell(0,8,txt('CONSTANCIA DE TRASLADO'),0,1,'C'); $pdf->Ln(8);
$pdf->SetFont('Arial','',11);
$pdf->MultiCell(0,6,txt('Se deja constancia que ' . strtoupper($est['nombre']) . ' (C.I. ' . $est['idusuario'] . ') ha sido autorizado(a) para traslado académico conforme a las normativas institucionales.'),0,'J');
$pdf->Ln(30); $pdf->SetFont('Arial','B',10); $pdf->Cell(0,4,txt('Departamento de Control de Estudios'),0,1,'C');
ob_end_clean(); $pdf->Output('I','Constancia_Traslado_' . $est['idusuario'] . '.pdf'); exit();
