...
<style>.btn-retirar{cursor:pointer}</style>
...
<div class="container-fluid py-2">
    <div class="row mb-2"><div class="col-12 d-flex justify-content-between">
        <h2 class="h4 mb-0">Detalles de Sección</h2><a href="gestion_seccion.php" class="btn btn-secondary btn-sm">← Volver</a>
    </div></div>
    <div class="row">
        <div class="col-lg-4">
            <div class="card shadow mb-2"><div class="card-header py-1"><h6 class="m-0">Información General</h6></div>
            ...
            <div class="card shadow"><div class="card-header py-1"><h6 class="m-0">Acciones</h6></div>
            <div class="card-body py-2">
                ...
                <?php endif; ?>
                <a href="horario_seccion.php?id=<?= $id_seccion ?>" class="btn btn-info btn-sm btn-block">Ver Horario Semanal</a>
            </div></div>
        </div>
        <div class="col-lg-8">
            <div class="card shadow mb-2"><div class="card-header py-1"><h6 class="m-0">Horarios de la Sección</h6></div>
            <div class="card-body py-2">
                <?php if (!empty($horarios)): ?>
                <div class="table-responsive"><table class="table table-bordered table-sm small">
                    <thead><tr><th>Día</th><th>Hora Inicio</th><th>Hora Fin</th><th>Aula</th><th>Materia</th><th>Docente</th></tr></thead>
                    <tbody><?php foreach ($horarios as $h): ?>
                        <tr><td><?= $h['dia_nombre'] ?></td><td><?= date('H:i', strtotime($h['hora_inicio'])) ?></td><td><?= date('H:i', strtotime($h['hora_fin'])) ?></td><td><?= $h['aula'] ?></td><td><?= $h['nombre_materia'] ?></td><td><?= $h['nombre_docente'] ?></td></tr>
                    <?php endforeach; ?></tbody>
                </table></div>
                <?php else: ?><div class="alert alert-info py-1 mb-0 small">No hay horarios definidos.</div><?php endif; ?>
            </div></div>
            <div class="card shadow"><div class="card-header py-1 d-flex justify-content-between"><h6 class="m-0">Estudiantes Asignados</h6><span class="badge badge-primary"><?= $estudiantes_inscritos ?></span></div>
            <div class="card-body py-2">
                <div class="table-responsive"><table class="table table-bordered table-sm small">
                    <thead><tr><th>Nombre</th><th>Cédula</th><th>Fecha Inscripción</th><th>Acciones</th></tr></thead>
                    <tbody><?php foreach ($estudiantes as $e): ?>
                        <tr><td><?= $e['nombre'] ?></td><td><?= $e['idusuario'] ?></td><td><?= $e['fecha_inscripcion'] ?></td>
                        <td><button class="btn btn-danger btn-sm btn-retirar" data-id="<?= $e['id'] ?>" data-nombre="<?= $e['nombre'] ?>"><i class="fas fa-user-minus"></i> Retirar</button></td></tr>
                    <?php endforeach; ?></tbody>
                </table></div>
            </div></div>
        </div>
...
$(document).ready(function(){
    $('.btn-retirar').click(function(){
        ...
        $('#modalConfirmacionBody').html('¿Está seguro de retirar a <strong>'+$(this).data('nombre')+'</strong>?');
